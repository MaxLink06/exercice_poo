...
class UIHelper
{
    ...
    public static function afficherLivresSousFormeDeTableau($livres)
    {
        ...
        echo str_pad("Titre", 40) . str_pad("Auteur", 30) . str_pad("Année", 10) . str_pad("Type", 20) . "\n";
        echo "\033[0m"; // Réinitialiser le style

        foreach ($livres as $livre) {
            echo str_pad($livre->getTitre(), 40);
            echo str_pad($livre->getAuthor(), 30);
            echo str_pad($livre->getPublicationYear(), 10);
            echo str_pad($livre->gettype(), 20);
